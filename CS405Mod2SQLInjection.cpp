...
// Developer: Steven Rodas | 9.15.2024 | contact@stevenrodas.com | CS-405 Secure Coding | SQl injection | Mod 2
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    //vector of values used in common SQl injection attacks
    std::vector<std::string> sql_injection_patterns = {" or ", " and ", "1=1", "1=2", "2=2", "'='", "\"=\"", ";", "--", "/*", "*/"};

    //converting the sql string to lowercase for easier comparison
    std::string sql_lowercase = sql;
    std::transform(sql_lowercase.begin(), sql_lowercase.end(), sql_lowercase.begin(), ::tolower);

    //checks for possible SQL injection based on patterns in the sql_injection_patterns vector above
    for (auto injection : sql_injection_patterns)
    {
        if (sql_lowercase.find(injection) != std::string::npos) // utilizing npos to check if the string is found or not
        {
            std::cout << "WARNING! SQL Injection has been detected in query." << std::endl;
            return false;
        }
    }
