...
// Developer: Steven Rodas | Date: 9/7/2024 | CS405 Module 1 Milestone | Numeric Overflows | steven.rodas@snhu.edu
...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        // checks for overflow of unsigned types
        if (std::numeric_limits<T>::is_integer && std::numeric_limits<T>::is_signed) 
        {
            // then checks for signed integers
            ...
            else if (std::numeric_limits<T>::is_integer && !std::numeric_limits<T>::is_signed)
            {
                // then checks for unsigned integers
    ...
    return result; // no overflow detected if returned
}
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        // checks for underflow of signed types
        if (std::numeric_limits<T>::is_integer && std::numeric_limits<T>::is_signed)
        {
            // signed integers
    ...
    return result; // no underflow detected if returned
}
